...
@Component({
  selector: 'app-ranking-details-newcomer',
  templateUrl: './ranking-details-newcomer.component.html',
  styleUrls: ['./ranking-details-newcomer.component.css'],
  providers:[RankingService]
})


export class RankingDetailsNewcomerComponent implements OnInit {
  photosWeek=[];
  photosWeekNow1=[];
  photosWeekNow2=[];
  photoNewcomer=[];
...
/*    /!*请求新人榜*!/
  ...
  findPhotoRankDate(){
    ...
    this.rankService.findPhotoRankDate(data3,res=>
    {
      ...
      for(var i=0;i<3;i++) {
        var photos=[];
        this.photoNewcomerNow1[i] = this.photoNewcomer[i];
        photos=this.photoNewcomerNow1[i].photoUrl.split(',');
        this.photoUrls[i]=photos[0];
      }
      for(var i=3;i<50;i++) {
        var photos2=[];
        this.photoNewcomerNow2[i-3] = this.photoNewcomer[i];
        photos2= this.photoNewcomerNow2[i-3].photoUrl.split(',');
        this.photoUrls[i]=photos2[0];
      }
    })
  }
